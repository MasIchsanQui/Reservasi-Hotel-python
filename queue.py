...
"}")
    ...
    def ClickedCetak(self):
        ...
        for row in rows:
            baris.enqueue(row)
            while not baris.is_empty():
                baris_data = baris.dequeue()
                id_transaksi, nama_tamu, tanggal_in, tanggal_out, no_kamar, num_days, jenis_kamar,total_cost_float, harga = baris_data
                tanggal_in = datetime.strftime("%Y-%m-%d")
                tanggal_out = datetime.strftime("%Y-%m-%d")
